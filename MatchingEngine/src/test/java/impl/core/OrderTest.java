...
public class OrderTest {
    ...
    public void testFillVolume() {
        Order order = new Order(BOOK_ID, 0, 0, 10.5, 1000, Side.BUY, 20, 0);
    ...
    public void testMatchBuySidePlaceOrderRequest() {
        Order sellOrder = new Order(BOOK_ID, 0, 0, 10, 1000, Side.SELL, 20, 0);
        PlaceOrderRequest buyOrderRequest = new PlaceOrderRequest(BOOK_ID, 1, 0, 11, Side.BUY, 10, 0, 1001);
        ITradeResponse trade = Order.matchOrderRequest(sellOrder, buyOrderRequest, mockTimestampProvider);
        assertEquals(trade, new TradeResponse(BOOK_ID, 1, 0, 0, 0, 10, 10, DUMMY_TIMESTAMP_SECONDS));
        assertTrue(buyOrderRequest.isMatched());
        assertFalse(sellOrder.isClosed());
        assertEquals(sellOrder.getUnfilledVolume(), 10);

        sellOrder = new Order(BOOK_ID, 0, 0, 10, 1000, Side.SELL, 20, 10);
        buyOrderRequest = new PlaceOrderRequest(BOOK_ID, 1, 0, 11, Side.BUY, 10, 0, 1001);
        trade = Order.matchOrderRequest(sellOrder, buyOrderRequest, mockTimestampProvider);
        assertEquals(trade, new TradeResponse(BOOK_ID, 1, 0, 0, 0, 10, 10, DUMMY_TIMESTAMP_SECONDS));
        assertTrue(buyOrderRequest.isMatched());
        assertTrue(sellOrder.isClosed());

        sellOrder = new Order(BOOK_ID, 0, 0, 20, 1000, Side.SELL, 100, 0);
        buyOrderRequest = new PlaceOrderRequest(BOOK_ID, 1, 0, 21, Side.BUY, 100, 0, 1001);
        trade = Order.matchOrderRequest(sellOrder, buyOrderRequest, mockTimestampProvider);
        assertEquals(trade, new TradeResponse(BOOK_ID, 1, 0, 0, 0, 20, 100, DUMMY_TIMESTAMP_SECONDS));
        assertTrue(buyOrderRequest.isMatched());
        assertTrue(sellOrder.isClosed());
    }
    ...
    public void testMatchSellSidePlaceOrderRequest() {
        Order buyOrder = new Order(BOOK_ID, 0, 0, 11, 1000, Side.BUY, 20, 0);
        PlaceOrderRequest sellOrderRequest = new PlaceOrderRequest(BOOK_ID, 1, 0, 10, Side.SELL, 10, 0, 1001);
        ITradeResponse trade = Order.matchOrderRequest(buyOrder, sellOrderRequest, mockTimestampProvider);
        assertEquals(trade, new TradeResponse(BOOK_ID, 0, 0, 1, 0, 11, 10, DUMMY_TIMESTAMP_SECONDS));
        assertTrue(sellOrderRequest.isMatched());
        assertFalse(buyOrder.isClosed());
        assertEquals(buyOrder.getUnfilledVolume(), 10);

        buyOrder = new Order(BOOK_ID, 0, 0, 11, 1000, Side.BUY, 20, 10);
        sellOrderRequest = new PlaceOrderRequest(BOOK_ID, 1, 0, 10, Side.SELL, 10, 0, 1001);
        trade = Order.matchOrderRequest(buyOrder, sellOrderRequest, mockTimestampProvider);
        assertEquals(trade, new TradeResponse(BOOK_ID, 0, 0, 1, 0, 11, 10, DUMMY_TIMESTAMP_SECONDS));
        assertTrue(sellOrderRequest.isMatched());
        assertTrue(buyOrder.isClosed());

        buyOrder = new Order(BOOK_ID, 0, 0, 21, 1000, Side.BUY, 100, 0);
        sellOrderRequest = new PlaceOrderRequest(BOOK_ID, 1, 0, 20, Side.SELL, 100, 0, 1001);
        trade = Order.matchOrderRequest(buyOrder, sellOrderRequest, mockTimestampProvider);
        assertEquals(trade, new TradeResponse(BOOK_ID, 0, 0, 1, 0, 21, 100, DUMMY_TIMESTAMP_SECONDS));
        assertTrue(sellOrderRequest.isMatched());
        assertTrue(buyOrder.isClosed());
    }
